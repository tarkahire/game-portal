...
function updateDenjiAnimation(pm, dt, moving, walkCycle) {
...
// Cinematic subtitle — anime-style caption that fades in, holds, fades out.
// Used by domain-expansion cut-ins (e.g. Megumi's Chimera Shadow Garden).
function showCinematicSubtitle(text, holdMs, opts) {
    opts = opts || {};
    const div = document.createElement('div');
    div.textContent = text;
    div.style.cssText = `position:fixed;left:50%;bottom:${opts.bottom || '18%'};` +
        `transform:translateX(-50%);z-index:60;pointer-events:none;` +
        `color:${opts.color || '#ffffff'};font-family:'Segoe UI',sans-serif;` +
        `font-weight:${opts.weight || 700};font-size:${opts.size || '2rem'};` +
        `letter-spacing:0.04em;white-space:nowrap;` +
        `text-shadow:0 0 12px rgba(124,77,255,0.75),0 2px 6px rgba(0,0,0,0.95);` +
...
function fruitAbility(slot) {
    ...
    if (id === 'yoh') {
        if (slot === 'z') {
            // ── CELESTIAL SLASH — massive spirit energy wave ──
            const fly = fpsCamera.flyHeight || 0;
            screenShake(0.6, 500);
            triggerHitstop(120);
            fovPunch(25, 0.35);
            ...
            // Oversoul arm — dramatic wind-up then huge sweep
            if (player._oversoulArm) {
                ...
                const sweepAnim = () => {
                    const t = Math.min((performance.now() - sweepStart) / 500, 1);
                    const ease = 1 - Math.pow(1 - t, 3);
                    arm.rotation.y = -1.5 + 3.0 * ease;
                    arm.rotation.z = 0.4 - 0.8 * ease;
                    if (t >= 1) { setTimeout(() => { arm.rotation.set(baseRx, 0, 0); player._oversoulSwinging = false; }, 200); return; }
                    requestAnimationFrame(sweepAnim);
                };
                requestAnimationFrame(sweepAnim);
            }

            // HUGE crescent wave projectile
            ...
            // Bright inner core
            wave.add(new THREE.Mesh(
                new THREE.TorusGeometry(2.0, 0.4, 4, 20, Math.PI),
                new THREE.MeshBasicMaterial({ color: '#ffffff', transparent: true, opacity: 0.6, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide })
            ));
            // Outer purple halo
            wave.add(new THREE.Mesh(
                new THREE.TorusGeometry(3.5, 0.08, 4, 24, Math.PI),
                new THREE.MeshBasicMaterial({ color: '#9c27b0', transparent: true, opacity: 0.4, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide })
            ));
            wave.add(new THREE.PointLight('#ff9800', 12, TILE * 10, 2));
            scene.add(wave);

            projectiles3D.push({
                mesh: wave, vx: fwdX * 14, vz: fwdZ * 14,
                damage: Math.round(player.damage * 4), owner: 'player',
                traveled: 0, range: 60
            });
            ...
            lightFlash(spawnX, EYE_HEIGHT, spawnZ, '#ff9800', 10, 500);
            groundRing(worldPx, worldPz, '#ff9800', 5, 700);
            groundRing(worldPx, worldPz, '#9c27b0', 3, 500);
        }

        else if (slot === 'x') {
            // ── SHINKUU BUDDHA GIRI — teleport-speed dash + devastating slash ──
            const fly = fpsCamera.flyHeight || 0;
            const dashDist = 8;
            screenShake(0.8, 600);
            triggerHitstop(150);
            ...
            player.invincible = performance.now() + 800;

            // Dash
            fpsCamera.safeMove(fpsCamera.posX + fwdX * dashDist, fpsCamera.posZ + fwdZ * dashDist, dungeon.map);
            showSpeedLines(600);

            const landX = fpsCamera.posX * TILE, landZ = fpsCamera.posZ * TILE;

            // Oversoul arm — wind up high, smash diagonal
            if (player._oversoulArm) {
                player._oversoulSwinging = true;
                const arm = player._oversoulArm;
                const baseRx = -Math.PI / 2;
                arm.rotation.set(baseRx - 0.6, -1.2, 0.7);
                const slashStart = performance.now();
                const slashAnim = () => {
                    const t = Math.min((performance.now() - slashStart) / 400, 1);
                    const ease = 1 - Math.pow(1 - t, 3);
                    arm.rotation.x = (baseRx - 0.6) + 1.5 * ease;
                    arm.rotation.y = -1.2 + 2.6 * ease;
                    arm.rotation.z = 0.7 - 1.4 * ease;
                    if (t >= 1) { setTimeout(() => { arm.rotation.set(baseRx, 0, 0); player._oversoulSwinging = false; }, 400); return; }
                    requestAnimationFrame(slashAnim);
                };
                requestAnimationFrame(slashAnim);
            }

            // Giant X slash — 4 layered slash planes
            for (let i = 0; i < 4; i++) {
                const slashGeo = new THREE.PlaneGeometry(8, 0.25);
                const slashMat = new THREE.MeshBasicMaterial({
                ...
                const slash = new THREE.Mesh(slashGeo, slashMat);
                slash.position.set(landX, EYE_HEIGHT + fly - 0.5 + i * 0.3, landZ);
                slash.rotation.set(0, yaw + Math.PI / 2, (i < 2 ? 0.5 : -0.5) + i * 0.1);
                scene.add(slash);
                const fadeStart = performance.now();
                const fadeSlash = () => {
                    const ft = (performance.now() - fadeStart) / 700;
                    if (ft >= 1) { scene.remove(slash); slash.geometry.dispose(); slash.material.dispose(); return; }
                    slashMat.opacity = (1 - ft) * 0.85;
                    slash.scale.set(1 + ft * 0.8, 1 + ft * 5, 1);
                    requestAnimationFrame(fadeSlash);
                };
                requestAnimationFrame(fadeSlash);
            }
            ...
            for (const e of enemies3D) {
                ...
                const toX = e.data.x - px, toZ = e.data.z - pz;
                const dot = toX * fwdX + toZ * fwdZ;
                if (dot > 0 && dot < dashDist + 3) {
                    const perp = Math.abs(toX * fwdZ - toZ * fwdX);
                    if (perp < 3.5) {
                        dealDamageToEnemy(e, Math.round(player.damage * 5));
                        e.data.x += fwdX * 2.5; e.data.z += fwdZ * 2.5;
                        e.mesh.position.set(e.data.x * TILE, 0, e.data.z * TILE);
                    }
                }
            }
            ...
            groundRing(landX, landZ, '#ffffff', 7, 800);
            groundRing(landX, landZ, '#9c27b0', 5, 600);
            groundDecal(landX, landZ, '#ff9800', 4, 3000);
            lightFlash(landX, EYE_HEIGHT, landZ, '#ffffff', 15, 600);
        }

        else if (slot === 'c') {
            // ── DOUBLE MEDIUM — Amidamaru possession, devastating 12-hit barrage ──
            const fly = fpsCamera.flyHeight || 0;
            screenShake(0.5, 2500);
            triggerHitstop(80);
            fovPunch(15, 0.4);
            screenFlash('rgba(156,39,176,0.4)', 800);
            player.invincible = performance.now() + 3000;

            // Massive spirit eruption on activation
            emitParticles(worldPx, EYE_HEIGHT + fly, worldPz, {
                color: ['#ff9800', '#ffffff', '#9c27b0', '#ffcc00'],
                count: 80, speed: 5, spread: 2.5,
                gravity: 0, life: 25, size: 0.2, sizeEnd: 0, drag: 0.95, upward: 5
            });
            groundRing(worldPx, worldPz, '#ff9800', 6, 1200);
            groundRing(worldPx, worldPz, '#9c27b0', 4, 800);
            lightFlash(worldPx, EYE_HEIGHT, worldPz, '#9c27b0', 10, 800);

            // 12 rapid slashes with escalating intensity
            for (let i = 0; i < 12; i++) {
                setTimeout(() => {
                    const dir = i % 2 === 0 ? 1 : -1;
                    const hitDist = 2 + Math.random() * 1.5;
                    const perpX = Math.cos(yaw), perpZ = -Math.sin(yaw);
                    const sx = worldPx + fwdX * hitDist + perpX * dir * (0.5 + i * 0.1);
                    const sy = EYE_HEIGHT + fly - 0.8 + Math.random() * 1.5;
                    const sz = worldPz + fwdZ * hitDist + perpZ * dir * (0.5 + i * 0.1);

                    // Bigger slash arcs that grow with each hit
                    ...
                    sMesh.rotation.set(Math.random() * 0.5 - 0.25, yaw + Math.PI / 2, dir * (0.4 + Math.random() * 0.5));
                    scene.add(sMesh);
                    ...
                    // Escalating damage per hit
                    const dmgMult = 1.0 + i * 0.2;
                    for (const e of enemies3D) {
                        if (!e.data.alive) continue;
                        if (Math.hypot(e.data.x - px, e.data.z - pz) < 5) {
                            dealDamageToEnemy(e, Math.round(player.damage * dmgMult));
                        }
                    }

                    screenShake(0.2 + i * 0.03, 80);
                    if (i === 11) triggerHitstop(100); // big freeze on last hit
                    if (i >= 9) lightFlash(sx, sy, sz, '#ffffff', 4, 100);

                    // Oversoul arm rapid slashing
                    if (player._oversoulArm) {
                        const arm = player._oversoulArm;
                        arm.rotation.y = dir * (0.8 + i * 0.05);
                        arm.rotation.z = dir * 0.4;
                        setTimeout(() => { arm.rotation.y = 0; arm.rotation.z = 0; }, 120);
                    }
                }, i * 150);
            }

            // Final explosion after barrage
            setTimeout(() => {
                screenShake(0.6, 300);
                ...
                groundRing(worldPx, worldPz, '#ffffff', 5, 500);
                lightFlash(worldPx, EYE_HEIGHT, worldPz, '#ffffff', 12, 400);
            }, 12 * 150 + 100);
        }

        else if (slot === 'v') {
            // ── FUMON TONKOU — earth-shaking spirit eruption ──
            const fly = fpsCamera.flyHeight || 0;
            screenShake(1.0, 2000);
            triggerHitstop(200);
            fovPunch(35, 0.5);
            screenFlash('rgba(255,255,255,0.8)', 1000);
            player.invincible = performance.now() + 3500;

            // Oversoul arm — dramatic overhead slam
            if (player._oversoulArm) {
                player._oversoulSwinging = true;
                const arm = player._oversoulArm;
                const baseRx = -Math.PI / 2;
                ...
                const slamAnim = () => {
                    const t = Math.min((performance.now() - slamStart) / 500, 1);
                    const ease = 1 - Math.pow(1 - t, 4);
                    arm.rotation.x = (baseRx - 0.8) + 2.5 * ease;
                    if (t >= 1) { setTimeout(() => { arm.rotation.x = baseRx; player._oversoulSwinging = false; }, 3000); return; }
            ...
            scene.add(pillar);
            pillar.add(new THREE.PointLight('#ff9800', 20, TILE * 15, 2));

            // Second purple pillar — bigger
            const pillar2 = new THREE.Mesh(
                new THREE.CylinderGeometry(0.5, 4, 18, 10, 1, true),
                new THREE.MeshBasicMaterial({ color: '#9c27b0', transparent: true, opacity: 0.4, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide })
            );
            pillar2.position.set(worldPx, fly, worldPz);
            scene.add(pillar2);

            // Third white inner pillar
            const pillar3 = new THREE.Mesh(
                new THREE.CylinderGeometry(0.3, 2, 22, 8, 1, true),
                new THREE.MeshBasicMaterial({ color: '#ffffff', transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide })
            );
            pillar3.position.set(worldPx, fly, worldPz);
            scene.add(pillar3);

            // 8 expanding shockwaves
            for (let r = 0; r < 8; r++) {
                setTimeout(() => {
                    groundRing(worldPx, worldPz, r % 3 === 0 ? '#ffffff' : r % 3 === 1 ? '#ff9800' : '#9c27b0', 4 + r * 2, 1000);
                    screenShake(0.3, 200);
                }, r * 150);
            }
            ...
            setTimeout(() => {
                emitParticles(worldPx, 1 + fly, worldPz, {
                    color: ['#9c27b0', '#ffffff', '#ff9800'],
                    count: 80, speed: 6, spread: 3,
                    gravity: 0, life: 25, size: 0.2, sizeEnd: 0, drag: 0.96, upward: 4
                });
                screenFlash('rgba(156,39,176,0.5)', 600);
            }, 500);

            // Damage + stun everything
            for (const e of enemies3D) {
                if (!e.data.alive) continue;
                const d = Math.hypot(e.data.x - px, e.data.z - pz);
                if (d < 10) {
            ...
            // DOT ticks with escalating particles
            let ticks = 0;
            ...
            setTimeout(() => {
                ...
                const fadePillars = () => {
                    ...
                    pillar2.material.opacity = (1 - ft) * 0.4;
                    pillar3.material.opacity = (1 - ft) * 0.3;
            ...
            groundDecal(worldPx, worldPz, '#ff9800', 6, 8000);
            lightFlash(worldPx, EYE_HEIGHT, worldPz, '#ffffff', 20, 1200);
        }

        else if (slot === 'f') {
            // ── SPIRIT DASH — Amidamaru ghost-speed burst ──
            const dashDist = 7;
            const fly = fpsCamera.flyHeight || 0;
            ...
            screenShake(0.25, 150);
            fovPunch(20, 0.15);
            screenFlash('rgba(255,152,0,0.3)', 200);
            ...
            for (let i = 0; i < 10; i++) {
                ...
                emitParticles(tx, EYE_HEIGHT + fly, tz, {
                    color: ['#ff9800', '#ffffff', '#9c27b0', '#ffb74d'],
                    count: 6, speed: 2, spread: 0.8,
                    gravity: 0, life: 14, size: 0.1, sizeEnd: 0, drag: 0.92
                });
            }
            ...
            showSpeedLines(500);
            lightFlash(worldPx, EYE_HEIGHT, worldPz, '#ff9800', 5, 250);
            groundRing(worldPx, worldPz, '#ff9800', 3, 300);
        }
    }
    ...
    if (id === 'megumi') {
        if (slot === 'z') { // Divine Dogs — summon white & black divine dogs
        ...
        else if (slot === 'v') { // Chimera Shadow Garden — domain (black puddle of darkness)
            const radius = 8;        // tile radius of the domain (gameplay)
            const wRad = radius * TILE; // world-unit radius (visual puddle)
            const duration = 7000;   // ms the darkness lingers after the cut-in

            // ── Cinematic intro — black-out wash, world freezes ──
            screenFlash('rgba(8,0,16,0.82)', 320);
            setTimeout(() => screenFlash('rgba(40,10,74,0.5)', 1700), 300);
            screenShake(0.55, 1300);
            triggerHitstop(120);
            fovPunch(14, 0.12);
            player._cutsceneActive = true;
            const HAND_DUR = 1750;
            ...
            // Swing the camera to a front view of Megumi forming the sign
            const camDist = 2.6;
            fpsCamera.setCinematic(
                worldPx + fwdX * camDist, 1.78, worldPz + fwdZ * camDist,
                worldPx, 1.12, worldPz, HAND_DUR
            ...
            // Subtitle lines lifted straight from the reference clip
            showCinematicSubtitle("Screw it!", 470, { size: '2rem' });
            setTimeout(() => showCinematicSubtitle("I'll do it!", 480, { size: '2.1rem' }), 520);
            setTimeout(() => showCinematicSubtitle("Domain Expansion:", 1500,
                { size: '1.45rem', color: '#cdbbff', bottom: '25%', weight: 600 }), 1060);
            setTimeout(() => showCinematicSubtitle("Chimera Shadow Garden", 2300,
                { size: '2.7rem', color: '#b39ddb' }), 1380);

            // Shadow-energy orb condensing between his clasped hands
            const handWX = worldPx + fwdX * 0.22, handWY = 1.30, handWZ = worldPz + fwdZ * 0.22;
            const ceOrb = new THREE.Group();
            const ceMat = new THREE.MeshBasicMaterial({
                color: '#3a1a8a', transparent: true, opacity: 0,
                blending: THREE.AdditiveBlending, depthWrite: false,
            });
            const ceCoreMat = new THREE.MeshBasicMaterial({
                color: '#b39ddb', transparent: true, opacity: 0,
                blending: THREE.AdditiveBlending, depthWrite: false,
            });
            ceOrb.add(new THREE.Mesh(new THREE.SphereGeometry(0.34, 12, 12), ceMat));
            ceOrb.add(new THREE.Mesh(new THREE.SphereGeometry(0.15, 10, 10), ceCoreMat));
            const ceLight = new THREE.PointLight('#7c4dff', 0, TILE * 4, 1.8);
            ceOrb.add(ceLight);
            ceOrb.position.set(handWX, handWY, handWZ);
            scene.add(ceOrb);
            ...
            const animateSign = () => {
                const st = Math.min((performance.now() - signStart) / HAND_DUR, 1);
                const raise = easeOutC(Math.min(st / 0.55, 1));
                const pressT = Math.max(0, (st - 0.55) / 0.45);
                const tremor = Math.sin(st * 55) * 0.045 * pressT;
                if (pmM?._rightArm) pmM._rightArm.rotation.set(-2.05 * raise, 0,  (0.82 + 0.12 * pressT) * raise + tremor);
                if (pmM?._leftArm)  pmM._leftArm.rotation.set(-2.05 * raise, 0, -(0.82 + 0.12 * pressT) * raise - tremor);
                const orbK = easeOutC(Math.min(st / 0.85, 1));
                ceOrb.scale.setScalar(0.25 + orbK * 1.0 + Math.sin(st * 40) * 0.04 * pressT);
                ceMat.opacity = orbK * 0.85;
                ceCoreMat.opacity = orbK;
                ceLight.intensity = orbK * 7;
                ...
                if (st < 1) { requestAnimationFrame(animateSign); return; }
                emitParticles(handWX, handWY, handWZ, {
                    color: ['#1a0a3a', '#7c4dff', '#b39ddb', '#ffffff'],
                    count: 26, speed: 6, spread: 0.4,
                    gravity: 0, life: 14, size: 0.14, sizeEnd: 0, drag: 0.92,
                });
                lightFlash(handWX, handWY, handWZ, '#7c4dff', 10, 260);
                scene.remove(ceOrb);
                ceMat.dispose(); ceCoreMat.dispose();
                ceOrb.children.forEach(c => c.geometry && c.geometry.dispose());
                fpsCamera.clearCinematic();
                player._cutsceneActive = false;
                if (!player || !player.alive || player.classId !== 'megumi') return;
                beginGarden();
            };
            requestAnimationFrame(animateSign);

            // ── The black puddle of darkness + its gameplay ──
            function beginGarden() {
                const domainEnd = performance.now() + duration;

                // Freeze every enemy caught in the darkness in place for the
                // whole domain (re-applied each tick so stragglers lock too).
                const trapInside = () => {
                    for (const e of enemies3D) {
                        if (!e.data.alive) continue;
                        if (Math.hypot(e.data.x - px, e.data.z - pz) < radius) {
                            e.data._domainTrapped = domainEnd;
                            e.mesh.traverse(c => { if (c.isMesh && c.material && c.material.emissive) {
                                c.material.emissive.set('#1a0633');
                                c.material._domainEmissive = true;
                            }});
                        }
                    }
                ...
                // Activation burst — heavy hit on everything caught inside
                for (const e of enemies3D) {
                    if (!e.data.alive) continue;
                    if (Math.hypot(e.data.x - px, e.data.z - pz) < radius) {
                        dealDamageToEnemy(e, Math.round(player.damage * 3));
                ...
                puddle.position.set(worldPx, 0, worldPz);
                puddle.scale.setScalar(0.02);
                scene.add(puddle);

                // Irregular pitch-black pool spanning the whole domain
                ...
                for (let i = 0; i < pp.count; i++) {
                    ...
                    if (Math.hypot(x, y) > 0.01) {
                        ...
                        const k = 1 + (Math.sin(ang * 5) * 0.5 + Math.sin(ang * 8 + 1.3) * 0.5) * 0.07;
                        pp.setXYZ(i, x * k, y * k, 0);
                    }
                }
                poolGeo.computeVertexNormals();
                const poolMat = new THREE.MeshBasicMaterial({
                    color: '#040208', transparent: true, opacity: 0,
                    depthWrite: false, side: THREE.DoubleSide,
                });
                ...
                const voidMesh = new THREE.Mesh(new THREE.CircleGeometry(wRad * 0.66, 48), voidMat);
                voidMesh.rotation.x = -Math.PI / 2;
                voidMesh.position.y = 0.07;
                puddle.add(voidMesh);

                // Faint violet rim — the only light the darkness gives off
                const rimMat = new THREE.MeshBasicMaterial({
                    color: '#7c4dff', transparent: true, opacity: 0,
                    blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide,
                });
                ...
                // Low purple light so the arena stays readable inside the dark
                const pLight = new THREE.PointLight('#3a1a6a', 0, TILE * 14, 1.6);
                pLight.position.set(0, 1.5, 0);
                puddle.add(pLight);

                groundDecal(worldPx, worldPz, '#0a0418', wRad, duration + 600);

                // Spread the puddle outward across the floor
                const spreadStart = performance.now();
                const SPREAD = 750;
                const spreadAnim = () => {
                    const t = Math.min((performance.now() - spreadStart) / SPREAD, 1);
                    const e2 = 1 - Math.pow(1 - t, 3);
                    puddle.scale.setScalar(0.02 + e2 * 0.98);
                    poolMat.opacity = e2 * 0.95;
                    voidMat.opacity = e2;
                    rimMat.opacity = e2 * 0.5;
                    pLight.intensity = e2 * 3;
                    if (t < 1) requestAnimationFrame(spreadAnim);
                };
                requestAnimationFrame(spreadAnim);

                // Shadow shikigami inside the garden — 2 divine dogs
                ...
                // Eruption as the darkness opens
                lightFlash(worldPx, 2.5, worldPz, '#7c4dff', 12, 700);
                emitParticles(worldPx, 0.6, worldPz, {
                    color: ['#000000', '#1a0a3a', '#7c4dff', '#0d1b5e'],
                    count: 70, speed: 6, spread: 2,
                    gravity: 0, life: 22, size: 0.18, sizeEnd: 0, drag: 0.95, upward: 3
                });
                for (let r = 0; r < 3; r++) {
                    setTimeout(() => groundRing(worldPx, worldPz, r % 2 === 0 ? '#1a0a3a' : '#7c4dff', radius - r * 1.4, 800), r * 90);
                }

                // ── Per-tick: keep frozen, damage, shadow lightning, bubbling ──
                const startTime = performance.now();
                let tickCount = 0;
                const tickInterval = setInterval(() => {
                    ...
                    if (elapsed >= duration) {
                        ...
                        const fadeOut = () => {
                            const ft = (performance.now() - fadeStart) / 700;
                            if (ft >= 1) { disposeGroup(puddle); releaseTint(); return; }
                            puddle.scale.setScalar(Math.max(0.02, 1 - ft));
                            poolMat.opacity = (1 - ft) * 0.95;
                            voidMat.opacity = (1 - ft);
                            rimMat.opacity = (1 - ft) * 0.5;
                            pLight.intensity = (1 - ft) * 3;
                    ...
                    tickCount++;
                    trapInside(); // keep the darkness locking everything inside
                    ...
                    // Rim pulse so the darkness "breathes"
                    rimMat.opacity = 0.4 + Math.sin(elapsed * 0.006) * 0.18;

                    // Every 4th tick — shadow lightning on the toughest enemy
                    if (tickCount % 4 === 0) {
                        ...
                        if (bestE) {
                            ...
                            const bMat = new THREE.MeshBasicMaterial({
                                color: '#9c4dff', transparent: true, opacity: 0.85,
                                blending: THREE.AdditiveBlending, depthWrite: false
                            });
                            ...
                            requestAnimationFrame(fadeBolt);
                            lightFlash(tx, 2, tz, '#9c4dff', 6, 200);
                            groundRing(tx, tz, '#7c4dff', 1.8, 300);
                            emitParticles(tx, 1, tz, {
                                color: ['#9c4dff', '#ffffff'], count: 12, speed: 4, spread: 0.6,
                    ...
                    // Black shadow tendrils bubbling up out of the puddle
                    const a = Math.random() * Math.PI * 2;
                    const r2 = Math.random() * radius * 0.92;
                    emitParticles(worldPx + Math.cos(a) * r2 * TILE, 0.3, worldPz + Math.sin(a) * r2 * TILE, {
                        color: ['#000000', '#1a0a3a', '#7c4dff'],
                        count: 4, speed: 1.6, spread: 0.4,
                        gravity: 0, life: 16, size: 0.12, sizeEnd: 0, drag: 0.95, upward: 2.5
                    });
                }, 250);
            } // ── end beginGarden() ──
        }
...
function yohOversoul() {
    ...
    // ── FLOATING SPIRIT ARM + KATANA (levitates around Yoh) ──
    ...
    // The entire oversoul floats as a scene-level group that follows the player
    const oversoul = new THREE.Group();
    oversoul.scale.setScalar(1.545); // scale up to match bigger Yoh (0.85/0.55 ratio)
    scene.add(oversoul);

    // ── GIANT SPIRIT ARM (segmented white armor with purple joints) ──
    const armGroup = new THREE.Group();

    // Shoulder — massive pauldron with purple trim + orange star
    const shoulder = new THREE.Mesh(new THREE.BoxGeometry(1.4, 1.0, 1.0), whiteMat);
    shoulder.position.y = 1.5; armGroup.add(shoulder);
    armGroup.add(new THREE.Mesh(new THREE.BoxGeometry(1.45, 0.08, 1.05), purpleMat).translateY(2.0));
    armGroup.add(new THREE.Mesh(new THREE.BoxGeometry(1.45, 0.08, 1.05), purpleMat).translateY(1.0));
    // Orange star emblem
    const emblem = new THREE.Mesh(new THREE.CircleGeometry(0.25, 6),
        new THREE.MeshBasicMaterial({ color: '#ff9800', side: THREE.DoubleSide }));
    emblem.position.set(0, 1.5, 0.52); armGroup.add(emblem);

    // Purple elbow joint sphere
    const elbow = new THREE.Mesh(new THREE.SphereGeometry(0.45, 8, 8), purpleMat);
    elbow.position.y = 0.2; armGroup.add(elbow);

    // Forearm — long armored segment
    const forearm = new THREE.Mesh(new THREE.BoxGeometry(0.9, 1.8, 0.8), whiteMat);
    forearm.position.y = -1.2; armGroup.add(forearm);
    armGroup.add(new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.06, 0.85), purpleMat).translateY(-0.35));
    armGroup.add(new THREE.Mesh(new THREE.BoxGeometry(0.95, 0.06, 0.85), purpleMat).translateY(-2.05));

    // Wrist joint
    const wrist = new THREE.Mesh(new THREE.SphereGeometry(0.35, 8, 8), purpleMat);
    wrist.position.y = -2.3; armGroup.add(wrist);

    // Giant gauntlet fist — gripping the sword
    const fist = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.6, 0.7), whiteMat);
    fist.position.set(0, -2.8, 0); armGroup.add(fist);
    // Knuckle plates
    armGroup.add(new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.1, 0.08), purpleMat).translateY(-2.55).translateZ(0.38));
    // Finger segments gripping
    for (let f = 0; f < 4; f++) {
        const finger = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.35, 0.15), whiteMat);
        finger.position.set((f - 1.5) * 0.18, -3.15, 0.15);
        finger.rotation.x = 0.5;
        armGroup.add(finger);
    }
    // Thumb
    const thumb = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.3, 0.15), whiteMat);
    thumb.position.set(0.45, -2.85, 0.2); thumb.rotation.z = -0.6;
    armGroup.add(thumb);

    // Arm glow light
    const armLight = new THREE.PointLight('#ffffff', 2, TILE * 6, 2);
    armLight.position.y = -1; armGroup.add(armLight);

    // Ghost aura shell around arm
    const armAura = new THREE.Mesh(new THREE.CylinderGeometry(1.2, 0.8, 5, 8, 1, true), ghostMat);
    armAura.position.y = -0.5; armGroup.add(armAura);
    ...
    // Hilt
    const hilt = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 0.8, 6), darkMat);
    swordGroup.add(hilt);
    for (let w = 0; w < 8; w++) {
        swordGroup.add(new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.075, 0.03, 6),
            new THREE.MeshStandardMaterial({ color: '#5a2a2a', roughness: 0.5 })).translateY(-0.3 + w * 0.09));
    }
    // Tsuba
    const tsuba = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.04, 8), whiteMat);
    tsuba.position.y = 0.44; swordGroup.add(tsuba);
    swordGroup.add(new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.05, 8), purpleMat).translateY(0.44));
    swordGroup.add(new THREE.Mesh(new THREE.SphereGeometry(0.08, 5, 5), whiteMat).translateY(-0.42));

    // Blade — long curved katana
    const bladeGeo = new THREE.BoxGeometry(0.14, 5.0, 0.025);
    const pos = bladeGeo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
        const y = pos.getY(i);
        pos.setZ(i, pos.getZ(i) + Math.pow(Math.max(0, y) / 2.5, 2) * 0.2);
    }
    pos.needsUpdate = true;
    bladeGeo.computeVertexNormals();
    const blade = new THREE.Mesh(bladeGeo, bladeWhiteMat);
    blade.position.y = 3.0; swordGroup.add(blade);

    // Spine
    swordGroup.add(new THREE.Mesh(new THREE.BoxGeometry(0.015, 5.0, 0.03),
        new THREE.MeshStandardMaterial({ color: '#888888', metalness: 0.6, roughness: 0.2 })).translateY(3.0).translateX(-0.065).translateZ(0.06));
    // Hamon
    swordGroup.add(new THREE.Mesh(new THREE.BoxGeometry(0.12, 5.0, 0.005),
        new THREE.MeshBasicMaterial({ color: '#ffffff', transparent: true, opacity: 0.12 })).translateY(3.0).translateX(0.01).translateZ(0.015));
    // Edge glow
    swordGroup.add(new THREE.Mesh(new THREE.BoxGeometry(0.008, 5.0, 0.035),
        new THREE.MeshBasicMaterial({ color: '#ffffff', transparent: true, opacity: 0.3 })).translateY(3.0).translateX(0.075).translateZ(0.1));
    // Tip
    swordGroup.add(new THREE.Mesh(new THREE.ConeGeometry(0.075, 0.3, 4), bladeWhiteMat).translateY(5.55).translateZ(0.2));

    // ── ENERGY RINGS on blade ──
    const swordRings = [];
    for (let i = 0; i < 7; i++) {
