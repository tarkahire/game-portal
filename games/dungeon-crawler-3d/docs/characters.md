...
- **Z — Idle Transfiguration** (`mahitoIdleTransfig`): short forward palm-touch cone (~3.2 tiles), `damage ×2.2`, **auto-marks** survivors for the E detonation + a ~0.9 s warp-stun. Core combo enabler.
- **X — Body Repel** (`mahitoBodyRepel`): 4.2-tile flesh shockwave — `damage ×1.8` + hard radial knockback, and Mahito hops 3 tiles backward with ~0.45 s i-frames. Panic/escape button.
- **C — Soul Multiplicity** (`mahitoSoulMultiplicity`): spawns 5 temporary transfigured-human minions (`imp` mesh, ~9 s life) that swarm enemies.
- **V — Self-Embodiment of Perfection** (`mahitoDomain`): Malevolent-Shrine-style cut-in (front camera, hand-sign, flesh orb, "Domain Expansion: / Self-Embodiment of Perfection" subtitles), then a fleshy crimson domain field (8-tile radius, 6.5 s). Traps every enemy inside (`_domainTrapped`) and **auto-transfigures them one at a time** via `explodeMahito` (the bulge→burst), chipping any remainder. Player invincible the whole sequence.
- **F — Shadow Step** (`mahitoShadowStep`): 6-tile fleshy dash, line damage `×1.5`, ~0.35 s i-frames + speed lines.
