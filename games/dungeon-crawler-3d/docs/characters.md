...
- **X — Body Repel** (`mahitoBodyRepel`): 4.2-tile shockwave — `damage ×1.8` + hard radial knockback. **Mahito slams both hands onto the floor** (arm anim + pastel slam ring); then **7 separate snake-like transfigured humans erupt from the floor**, fanned (~98°) toward where he's aiming, and strike forward. Each is a smooth pale-pastel tapered body with a **single gaping dark mouth at the head — no hands, no eyes**; per-snake varied reach/whip/head-lift, rises out of the ground in an arc then retracts. Mahito hops 3 tiles backward with ~0.45 s i-frames.
