...
### H — Great Serpent (Megumi-only key)
...
- **Body**: continuous lofted tube (no bead gaps) — `buildLoftedTube(spinePoints, radii, radialSegs, colorFn)` builds a Catmull-Rom spline + cross-section ring sampling along the curve. 18 spine points, 14 radial segments, taper 0.62 (thick neck/upper body) → 0.06 (fine tail tip). **Vertex-colored**: olive-yellow scales on the upper body with cream belly underneath; the tail end transitions into alternating black-and-white bands (top) with a white belly. Whole serpent scaled 2.2× — wider, longer, more imposing than before.
