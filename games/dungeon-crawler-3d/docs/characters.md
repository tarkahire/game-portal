...
- **X — Body Repel** (`mahitoBodyRepel`): 4.2-tile flesh shockwave — `damage ×1.8` + hard radial knockback — that now erupts **6 grotesque multi-segment transfigured-flesh snake-heads** (yellow-eyed, writhing) lashing outward from incompatible fused souls, canon-accurate; Mahito hops 3 tiles backward with ~0.45 s i-frames. Panic/escape button.
