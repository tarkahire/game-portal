...
- **X — Body Repel** (`mahitoBodyRepel`): 4.2-tile shockwave — `damage ×1.8` + hard radial knockback — that erupts a **dense barrage of 14 pale pastel transfigured-flesh ARMS** (8-segment limb tipped with a clawed grasping hand — palm, 5 fingers, a small gaping dark mouth, **no eyes**) lashing outward in every direction with staggered eruption + per-limb varied reach/height/whip; Mahito hops 3 tiles backward with ~0.45 s i-frames. Panic/escape button.
