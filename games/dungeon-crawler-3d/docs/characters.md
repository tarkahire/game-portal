...
- **X — Body Repel** (`mahitoBodyRepel`): 4.2-tile shockwave — `damage ×1.8` + hard radial knockback — that erupts **6 pale pastel transfigured-flesh limbs** (small gaping dark mouth at the tip, **no eyes**, writhing) lashing outward from incompatible fused souls; Mahito hops 3 tiles backward with ~0.45 s i-frames. Panic/escape button.
