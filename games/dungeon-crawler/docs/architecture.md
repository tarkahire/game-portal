...
| File | Lines | Purpose |
|------|-------|---------|
| `index.html` | ~278 | Game page, all UI screens (title, class select, lobby, shop, stats, game over, inventory, pause) |
| `game.js` | ~3935 | All game logic: classes, enemies, bosses, combat, rendering, dungeon gen, loot, UI |
| `network.js` | ~482 | PeerJS WebRTC multiplayer: room management, state sync, input relay |
| `style.css` | ~202 | Dark themed styling for all menus, overlays, class cards, lobby |
...
4. **Class Definitions** — `CLASSES` object with 53 character classes
...
17. **Character Drawing** — 53 unique `draw*` functions for each class
