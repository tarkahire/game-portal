-- seed_server_v2.sql — Fixed hex spiral generation
-- Run each section separately in Supabase SQL Editor

-- ============================================
-- 1. CLEAN UP OLD DATA
-- ============================================
DELETE FROM tile_adjacency WHERE server_id = (SELECT id FROM game_servers WHERE name = 'European Theater #1' LIMIT 1);
DELETE FROM tiles WHERE server_id = (SELECT id FROM game_servers WHERE name = 'European Theater #1' LIMIT 1);

-- ============================================
-- 2. REGENERATE 999 HEX TILES (fixed spiral)
-- ============================================
DO $$
DECLARE
  v_server_id UUID;
  ...
  -- FIXED: Ring walk directions (SE, S, SW, NW, N, NE)
  -- These walk AROUND a ring, not outward from center
  dir_q INT[] := ARRAY[1, 0, -1, -1, 0, 1];
...
BEGIN
  ...
  FOR v_ring IN 1..18 LOOP
    -- Start position for each ring: (0, -ring) = top of hexagon
    v_q := 0; v_r := -v_ring;
    FOR v_side IN 0..5 LOOP
      FOR v_step IN 1..v_ring LOOP
        ...
        -- Move to next position along the ring side
...
-- ============================================
-- 3. REGENERATE TILE ADJACENCY
-- ============================================
DO $$
DECLARE
  v_server_id UUID;
  -- These are the 6 hex NEIGHBOR directions (not ring walk directions)
