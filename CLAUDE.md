...
- **Concept**: JJK-inspired open-world action RPG. Name sign-in + `localStorage` autosave (behind a swappable `SaveAdapter` so a Supabase/Resend backend can drop in later). Hilly heightmap world, one town with **non-enterable houses** + three walk-up **human NPCs** (mission board / cursed-tool smith / Jujutsu High contact), grade-scaled curse spawn director, M1 melee + dash, **3 selectable cursed techniques** (Z+X each), quests (exorcism + reusable Grade Exam), XP/levels, **full grade climb Grade 4 → Special Grade**, gold smith upgrades, WebAudio SFX + juicy VFX + camera shake, HUD + minimap.
- **Cursed techniques** (chosen at sign-in, `save.technique`, keys `strike`/`dismantle`/`flame` kept stable): re-creations of dungeon-crawler-3d character kits — **Megumi/Ten Shadows** (Z Divine Dogs hound-summons + `allies` system, X Nue), **Sukuna** (Z Dismantle slash volley, X Cleave arc), **Todo** (Z Black Flash blink-detonation, X Boulder Kick). Re-created in this lighter engine, **not** a literal DC-3D code port.
- **Status**: Playable — MVP + Updates 1–3 (2026-05-19). Built clean (the original "fork anime-battle-royale" plan was dropped). Monolithic `src/main.js` + `src/save/`. Full design + doc framework in `games/jujutsu-cursed-roads/docs/` (`design.md`/`devlog.md` first); roadmap in `docs/todo.md`.
...
- **Features**: First-person 3D dungeon crawler with anime character system; 10 playable characters with custom 3D models, unique abilities, and detailed VFX; Z/X/C/V/F abilities + Q oversoul/dash; M1 4-hit melee combo (4th hit = bisect finisher); procedural dungeon generation (120x120 map, rooms 9-16 tiles, 3-wide corridors); **abandoned-lab horror visual style** (grimy cracked/bloodstained floor + concrete walls, oppressive murky fog, dim sickly fill light, failing red/toxic-green emergency lamps that flicker & brown out — see devlog 2026-05-19 retheme; menus/HUD still neon); hitstop, floating damage numbers, speed lines, FOV punch; boss encounters; 5-slot ability cooldown HUD; 1st/3rd person camera toggle (T key); no ceiling (open sky); no screen shake; no dash cooldown; summoned divine dog wolves (Megumi); **fleshy organic horror enemies** with drool/tendrils/glowing-cracks/exposed-ribcages; **online co-op MVP** (full-screen view per device, see other players as 3D meshes, ~30Hz position sync with interpolation, host-authoritative dungeon, debug overlay)
- **Controls**:
